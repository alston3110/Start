...
#ONLY_P1 : TRUE is only Partition 1.don't care ROOTFS_SIZE
#MBR_SIZE : P0
#ROOTFS_SIZE : P1
#REV_SIZE : Reserve
# ==================================
...
efmlen=${#efmstr}
efmstart=$(expr $efmlen - 8)
efmver=${efmstr:$efmstart:8}
...
if [ "$efmver" != ${MCU_VER} ]; then
    ...
    efmstart=$(expr $efmlen - 8)
    efmver=${efmstr:$efmstart:8}
    print "efm32 update version is $efmver"
    $efm32cmd 2 12 -sg 0
fi
fi
fi
...
ROOTFS=`ls /data/flash/ubuntu-*`
LIBS=`ls /data/flash/lib.*`
...
if [ "$ONLY_P1"x != "TRUEx" ] ;  then
    ...
    REV_SIZE=`expr $REV_SIZE \* 1024 \* 1024`
    ROOTFS_SIZE=`expr $TOTAL_SIZE - $MBR_SIZE - $REV_SIZE`
fi
...
if [ "$ROOTFS"x != "x" ] ;  then
	...
	print "  tar --numeric-owner -xpf ${ROOTFS}"
	tar --numeric-owner -xpf ${ROOTFS} -C /mnt/
	sync
	sync
	if [ "$LIBS"x != "x" ] ; then
