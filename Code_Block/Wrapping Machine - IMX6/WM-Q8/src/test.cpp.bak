...
extern void iReport_ItemGet( void ) ;
/* ------------------------------------------------------------------------- */
enum __TEST_STATE__ {
    PROC_RS232_START  ,
    PROC_DATA_INIT ,
...
void vTest_ConnectPCBA( void )
{
    if ( iTestCount == 0 )
    {
        ...
        Edit_Enable( GetDlgItem( hRunDlgWnd , IDC_CHECKBOX1 ) , FALSE );
    }
...
void vTest_StartMp( void )
{
    ...
    {
        if ( iTestAuto ){
            vStatus_Show("START MP TEST AUTO-RUN !! ") ;
            SetWindowText( GetDlgItem( hRunDlgWnd , IDC_EDIT_INPUT2 ) , "AUTO-RUN" ) ;
            Edit_Enable( GetDlgItem( hRunDlgWnd , IDC_EDIT_INPUT2 ) , FALSE ) ;
            cTestNum[0]= '0' ;
            cTestNum[1]= '0' ;
            cTestNum[2]= 0x0d ;
...
void vTest_MP_TEST( void )
{
    ...
    if(ListView_GetCheckState( GetDlgItem( hRunDlgWnd , IDC_CHECKBOX1 ) , 0 ))
    {
        vStatus_Show( "START AUTO-RUN !!!!" ) ;
        iTestState = PROC_CONNECT_PCBA ;
        iTestAuto = 1 ;
        iTestCount = 0 ;
        return;
...
void vTest_MP_TEST_Select( void )
{
    if ( iTestCount == 0 )
    {
        if ( ! iTestAuto ){
            ...
            SetWindowText( GetDlgItem( hRunDlgWnd , IDC_STATIC_INPUT2 ) , "測試項目" ) ;
            SetWindowText( GetDlgItem( hRunDlgWnd , IDC_EDIT_INPUT2 ) , "" ) ;
            Edit_Enable( GetDlgItem( hRunDlgWnd , IDC_EDIT_INPUT2 ) , TRUE ) ;
            SetFocus( GetDlgItem( hRunDlgWnd , IDC_EDIT_INPUT2 ) ) ;
        }
...

   {
        ...
        iStrLen = GetWindowTextLength( GetDlgItem( hRunDlgWnd , IDC_EDIT_INPUT2 ) ) ;
        if ( iStrLen )
        {
            iReport_ItemGet();
            ...
            if ( iStrLen == DEF_TEST_NUMBER_LEN )
            {
                ...
                GetWindowText( GetDlgItem( hRunDlgWnd , IDC_EDIT_INPUT2 ) , edittxt , iStrLen+1 ) ;
                ShowWindow(GetDlgItem(hRunDlgWnd, IDC_BUTTON_RESTART), SW_HIDE);
                Edit_Enable( GetDlgItem( hRunDlgWnd , IDC_EDIT_INPUT2 ) , FALSE ) ;
...
void vTest_Exit( void )
{
    if ( iTestCount == 0 )
    {
        SetWindowText( GetDlgItem( hRunDlgWnd , IDC_BUTTON_RESTART ) , "換下一台" ) ;
