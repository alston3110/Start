...
void vTest_StartMp( void )
{
    ...
    }else if ( stcTestCheckData[1].iState  )
    {
        ...
        SetWindowText( GetDlgItem( hMainWnd , IDC_BUTTON_YES_NO_FAIL ) , "結束" ) ;
        SetWindowText( GetDlgItem( hMainWnd , IDC_BUTTON_YES_NO_OK ) , "繼續" ) ;
        ShowWindow(GetDlgItem(hMainWnd, IDC_BUTTON_YES_NO_FAIL), SW_SHOW);
        ShowWindow(GetDlgItem(hMainWnd, IDC_BUTTON_YES_NO_OK), SW_SHOW);
        iTestProcess = 3;
...
void vTest_MP_TEST( void )
{
    ...
    {
        SetWindowText( GetDlgItem( hMainWnd , IDC_BUTTON_RESTART ) , "自動測試" ) ;
...
void vTest_MP_TEST_Select( void )
{
    if ( iTestCount == 0 )
    {
        if ( ! iTestAuto ){
            ...
            SetWindowText( GetDlgItem( hMainWnd , IDC_STATIC_INPUT2 ) , "測試項目" ) ;
...
void vTest_MP_TEST_testing( void )
{
    ...
    if ((iTestItem > 0) && ( iTestShowItem == 0)){
            ...
            switch (iTestItem) {
                case 1 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "LED 測試" ) ;
                    break;
                case 2 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "GPIO 測試" ) ;
                    break;
                case 3 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "AUDIO 測試" ) ;
                    break;
                case 4 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "LVDS 測試" ) ;
                    break;
                case 5 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "3AXIS 測試" ) ;
                    break;
                case 6 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "TOUCHPANEL 測試" ) ;
                    break;
                case 7 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "USB 測試" ) ;
                    break;
                case 8 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "WIFI 測試" ) ;
                    break;
                case 9 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "ETH0 測試" ) ;
                    break;
                case 10 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "ETH1 測試" ) ;
                    break;
                case 11 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "RS232 測試" ) ;
                    break;
                case 12 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "DDR 測試" ) ;
                    break;
                case 13 :
                    SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "SDCARD 測試" ) ;
                    break;
                default :
                    break;
            }
...
void vTest_MP_TEST_show( void )
{
    ...
    if (iTestEnd){
            if( (iTestItem == 13) && iTestAuto ){
                vStatus_Show( " MP 測試已完成, 請更換下一片PCBA" ) ;
                SetWindowText( GetDlgItem( hMainWnd , IDC_EDIT_INPUT2 ) , "" ) ;
                iTestState = PROC_EXIT ;
            }else{
...
void vTest_Exit( void )
{
    if ( iTestCount == 0 )
    {
        SetWindowText( GetDlgItem( hMainWnd , IDC_BUTTON_RESTART ) , "換下一台" ) ;
        ShowWindow(GetDlgItem(hMainWnd, IDC_BUTTON_RESTART), SW_SHOW);
        iTestTimeOut = 0 ;
    }
